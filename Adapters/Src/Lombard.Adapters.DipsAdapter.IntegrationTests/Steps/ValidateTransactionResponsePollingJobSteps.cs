...
{
    [Binding]
    public class ValidateTransactionResponsePollingJobSteps
    {
        [Given(@"there are Transaction Validation database rows for batch number (.*) in DipsQueue database")]
        ...
        [Given(@"there are Transaction Validation database rows for batch number (.*) in DipsNabChq database")]
        ...
        [When(@"wait for (.*) seconds until ValidateTransactionResponsePollingJob executed")]
        public void When1(int waitSeconds)
        {
            Thread.Sleep(waitSeconds * 1000);
        }

        private ValidateBatchTransactionResponse response;

        [Then(@"a ValidateBatchTransactionResponse with batch number (.*) is added to the exchange and contains these vouchers:")]
        ...
        [Then(@"a ValidateBatchTransactionResponse with batch number (.*) contains this voucher batch:")]
        public void Then2(string batchNumber, Table table)
        {
            table.CompareToInstance(response.voucherBatch);
        }

        [Then(@"with these validate transaction voucher values:")]
