Feature: GenerateBulkCreditResponsePollingJob
	Given a valid record in queue database
	And valid vouchers in database
	When wait and a GenerateBulkCreditResponsePollingJob executed
	Then a GenerateBatchBulkCreditResponse including vouchers is added to the exchange

@GenerateBulkCreditResponsePollingJob
Scenario: GenerateBulkCredit response for a clean voucher
	Given there are Generate Bulk Credit Voucher database rows for batch number 58300013 in DipsQueue database
	| Field      | Value          |
	| S_LOCATION | BulkCreditDone |
	| S_LOCK     | 0              |
	| S_CLIENT   | NabChq         |
	| S_JOB_ID   | NabChqPod      |
	| S_MODIFIED | 0              |
	...
	| S_STIME    | 12:30:00       |
	| RoutingKey | NECL           |
	And there are Generate Bulk Credit database rows for batch number 58300013 in DipsNabChq database
	...
	When wait for 10 seconds until GenerateBulkCreditResponsePollingJob executed
	Then a GenerateBulkCreditResponse with batch number 58300013 is added to the exchange and contains these generated vouchers:
	...
	And a GenerateBulkCreditResponse with batch number 58300013 is added to the exchange and contains these non-generated vouchers:
	| documentReferenceNumber | bsbNumber | accountNumber | auxDom | extraAuxDom | transactionCode | amount  | 
	| 583000026               | 013812    | 256902729     | 001193 |             | 50              | 45.67   | 
	And a GenerateBulkCreditResponse with batch number 58300013 contains this voucher batch:
	| Field              | Value      |
	| workType           | NABCHQ_POD |
	| processingState    | SA         |
