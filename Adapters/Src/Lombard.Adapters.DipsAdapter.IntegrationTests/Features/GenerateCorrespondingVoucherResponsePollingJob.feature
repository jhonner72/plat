Feature: GenerateCorrespondingVoucherResponsePollingJob
	Given a valid record in queue database
	And valid vouchers in database
	When wait and a GenerateCorrespondingVoucherResponsePollingJob executed
	Then a GenerateCorrespondingVoucherBatchResponse including vouchers is added to the exchange

@GenerateCorrespondingVoucherResponsePollingJob
Scenario: GenerateCorrespondingVoucher response for a clean voucher
	Given there are Generate Corresponding Voucher database rows for batch number 58300013 in DipsQueue database
	| Field      | Value                            |
	...
	| S_STIME    | 12:30:00                         |
	| RoutingKey | NCST                             |
	And there are Generate Corresponding Voucher database rows for batch number 58300013 in DipsNabChq database
	...
	When wait for 10 seconds until GenerateCorrespondingVoucherResponsePollingJob executed
	Then a GenerateCorrespondingVoucherResponse with batch number 58300013 is added to the exchange and contains these generated vouchers:
	...
	And a GenerateCorrespondingVoucherResponse with batch number 58300013 is added to the exchange and contains these non-generated vouchers:
	| documentReferenceNumber | bsbNumber | accountNumber | auxDom | extraAuxDom | transactionCode | amount  | 
	| 583000026               | 013812    | 256902729     | 001193 |             | 50              | 45.67   | 
	And a GenerateCorrespondingVoucherResponse with batch number 58300013 contains this voucher batch:
	| Field              | Value      |
	| workType           | NABCHQ_POD |
	| processingState    | SA         |
