...
namespace Lombard.Adapters.DipsAdapter.Jobs
{
    ...
    public class GenerateCorrespondingVoucherResponsePollingJob : IJob
    {
        private readonly ILifetimeScope component;
        private readonly IDipsDbContext dbContext;
        private readonly IExchangePublisher<GenerateCorrespondingVoucherResponse> responseExchange;
                    ...
                    foreach (var completedBatch in completedBatches)
                    {
                        ...
                        using (var tx = dipsDbContext.BeginTransaction())
                        {
                            try
                            {
                                ...
                                var originalVouchers = dipsDbContext.NabChqPods
                                    .Where(
                                        v =>
                                ...
                                var generatedVouchers = dipsDbContext.NabChqPods
                                    .Where(
                                        v =>
                                            v.S_BATCH == batchNumber && v.S_DEL_IND != "  255" &&
                                            v.isGeneratedVoucher == "1" &&
