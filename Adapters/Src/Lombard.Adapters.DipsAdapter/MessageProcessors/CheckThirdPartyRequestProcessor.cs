...
{
    public class CheckThirdPartyRequestProcessor : IMessageProcessor<CheckThirdPartyBatchRequest>
    {
        private readonly ILifetimeScope component;
        private readonly IDipsDbContext dbContext;
        private readonly IMapper<CheckThirdPartyBatchRequest, DipsQueue> dipsQueueMapper;
        private readonly IMapper<CheckThirdPartyBatchRequest, IEnumerable<DipsNabChq>> dipsVoucherMapper;
        private readonly IMapper<CheckThirdPartyBatchRequest, IEnumerable<DipsDbIndex>> dipsDbIndexMapper;
        private readonly IImageMergeHelper imageMergeHelper;

        public CheckThirdPartyBatchRequest Message { get; set; }
        ...
        public async Task ProcessAsync(System.Threading.CancellationToken cancellationToken, string correlationId, string routingKey)
        {
            ...
            Log.Information("Processing CheckThirdPartyRequest '{@request}', '{@correlationId}'", request, correlationId);
                    ...
                    using (var dipsDbContext = dbContext ?? lifetimeScope.Resolve<IDipsDbContext>())
                    {
                        using (var tx = dipsDbContext.BeginTransaction())
                        {
                            try
                            {
                                ...
                                Log.Information(
                                    "Successfully processed CheckThirdPartyRequest '{@batchNumber}', '{@jobIdentifier}'",
            ...
            {
                Log.Error(ex, "Error processing CheckThirdPartyRequest {@CheckThirdPartyRequest}", request);
                throw;
            }
        }
    }
