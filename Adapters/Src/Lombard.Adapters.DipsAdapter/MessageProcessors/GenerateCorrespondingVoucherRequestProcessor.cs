...
{
    public class GenerateCorrespondingVoucherRequestProcessor : IMessageProcessor<GenerateCorrespondingVoucherRequest>
    {
        private readonly ILifetimeScope component;
        private readonly IDipsDbContext dbContext;
        private readonly IMapper<GenerateCorrespondingVoucherRequest, DipsQueue> dipsQueueMapper;
        private readonly IMapper<GenerateCorrespondingVoucherRequest, IEnumerable<DipsNabChq>> dipsVoucherMapper;
        private readonly IMapper<GenerateCorrespondingVoucherRequest, IEnumerable<DipsDbIndex>> dipsDbIndexMapper;
        private readonly IAdapterConfiguration adapterConfiguration;

        public GenerateCorrespondingVoucherRequest Message { get; set; }
        ...
        public async Task ProcessAsync(System.Threading.CancellationToken cancellationToken, string correlationId, string routingKey)
        {
            ...
            try
            {
                ...
                SetBatchNumber(request, generatedBatchNumber);
        ...
        /// </summary>
        private void SetBatchNumber(GenerateCorrespondingVoucherRequest request, string batchNumber)
        {
            try
            {
                request.generateVoucher.Select(v => { v.voucherBatch.scannedBatchNumber = batchNumber; return v; }).ToList();
            }
            catch (Exception ex)
            {
