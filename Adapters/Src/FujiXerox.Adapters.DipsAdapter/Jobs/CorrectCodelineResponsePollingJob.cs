...
{
    public class CorrectCodelineResponsePollingJob : PollingJob
    {
        public CorrectCodelineResponsePollingJob(IAdapterConfiguration configuration, ILogger log, RabbitMqExchange exchange) 
                    ...
                    foreach (var completedBatch in completedBatches)
                    {
                        ...
                        using (var tx = dbContext.BeginTransaction())
                        {
                            try
                            {
                                ...
                                if (vouchers.Count > 0)
                                {
                                    var firstVoucher = vouchers.First(v => !ResponseHelper.ParseStringAsBool(v.isGeneratedVoucher));
                                    ...
                                    var batchResponse = new CorrectBatchCodelineResponse
                                    {
                                        ...
                                        voucher = vouchers.Select(v => new CorrectCodelineResponse
                                        {
                                            ...
                                            repostFromProcessingDate = ResponseHelper.ParseDateField(v.repostFromProcessingDate),
