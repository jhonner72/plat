 ...
 */
@WebServlet(name = "runptqacdcquery", urlPatterns = { "/runptqacdcquery" })
public class RunPTQACdcQueryServlet extends BasePTQAServlet {
...
//	public static final String FAIL_UNKNOWN_ERR="-3";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		boolean isAmtOtherwiseCodeline=false; //IMPORTANT: DON'T CHANGE THIS VALUE. IT MUST BE TRUE FOR AMOUNT PROCESSING
		...
		try{
			...
			if(no_of_records > 0){			
				...
				String fileName=gpvastx.writeToXML(date,isAmtOtherwiseCodeline,PTQAProperties.xmlnameprefixforcdc);
				
				if(fileName != null && fileName.trim().length() > 0){

					CommonUtils.writeToLog("Setting ServletContext param ["+CDC_XMLFILENAME+"] value to ["+fileName+"]", CommonUtils.debug, null);	
					getServletContext().setAttribute(CDC_XMLFILENAME, fileName);
					
					CommonUtils.writeToLog("Setting ServletContext param ["+CDC_COUNTER+"] value to [1]", CommonUtils.debug, null);
					getServletContext().setAttribute(CDC_COUNTER, new Integer(1));		
					
					CommonUtils.writeToLog("Setting ServletContext param ["+CDC_NO_OF_RECORDS+"] value to ["+no_of_records+"]", CommonUtils.debug, null);
					getServletContext().setAttribute(CDC_NO_OF_RECORDS, new Integer(no_of_records));					
					...
					getServletContext().setAttribute(CDC_COUNTER, new Integer(0));
					getServletContext().setAttribute(CDC_NO_OF_RECORDS, new Integer(-1));	
					getServletContext().setAttribute(CDC_XMLFILENAME, "");
