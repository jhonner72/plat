...
public class WorkflowSuspensePoolMethod implements IDfMethod
{
	...
	public int execute(Map params, PrintWriter pstream) throws Exception {
    	...
    	writeToLog("{WorkflowSuspensePoolMethod}{execute}Starting ....",debug,null);
    	m_writer = pstream;
        ...
        try {
            ...
            writeToLog("{WorkflowSuspensePoolMethod}{execute}Getting voucher(s)",debug,null);
                                 ...
                                 if (docIdObj!=null) 
                                    {
                                      ...
                                      if(actName.equalsIgnoreCase("End")){
                                    	//just complete to go to end 
                                    	  ...
                                    	  writeToLog("{WorkflowSuspensePoolMethod}{execute}workflow complete",debug,null);
                                      }else{
                                    	  if (doc.isCheckedOut()){
                                    		  writeToLog("{WorkflowSuspensePoolMethod}{execute}Unable to checkout",debug,null);
                                    		  throw new Exception ("Document is locked or already checkout!!");

                                    	  }else{
                                    		  writeToLog("{WorkflowSuspensePoolMethod}{execute}Check out called",debug,null);
                                    		  doc.checkout();  
                                    	  }                                  
	                                      ...
	                                      if(actName.equalsIgnoreCase("NotifyIncomingPool")){
	                                    	//just complete to go to end 
	                                    	  ...
	                                    	  writeToLog("{WorkflowSuspensePoolMethod}{execute}workflow aborted",debug,null);
	                                      }
	                                      doc.setString("fxa_surplus_item_flag","false");                                      
	                                      //doc.save();
	                                      if (doc.isCheckedOut()){
	                                    	  writeToLog("{WorkflowSuspensePoolMethod}{execute}Check in called",debug,null);
	                                    	  doc.checkin(false,"CURRENT");
	                                    	  writeToLog("{WorkflowSuspensePoolMethod}{execute}New ID" +
	                                    			  doc.getObjectId().toString(),debug,null);
	                                      }else{
	                                    	  writeToLog("{WorkflowSuspensePoolMethod}{execute}Unable to checkin",debug,null);
	                                    	  throw new Exception ("Document should be checked in and checked out in same session !!");
	                                      }

	                                      writeToLog("{WorkflowSuspensePoolMethod}{execute}Update attribute",debug,null);
	                                      // Update audit table
        ...
        } finally
        {
            if ( session != null ){
                ...
                writeToLog("{WorkflowSuspensePoolMethod}{execute}Ending ....",debug,null);
            }
