	...
	public class WorkflowDocRetrievalMethod implements IDfMethod
	{
		...
		public int execute(Map params, PrintWriter pstream) throws Exception {
	    	...
	    	writeToLog("{WorkflowDocRetrievalMethod}{execute}Starting ....",debug,null);
	    	m_writer = pstream;
	        ...
	        try {
	            ...
	            writeToLog("{WorkflowDocRetrievalMethod}{execute}Getting voucher(s)",debug,null);
	                                 ...
	                                 if (docIdObj!=null) 
	                                    {
	                                      ...
	                                      if(actName.equalsIgnoreCase("End")){
	                                    	//just complete to go to end 
	                                    	  ...
	                                    	  writeToLog("{WorkflowDocRetrievalMethod}{execute}workflow complete",debug,null);
	                                      }else{
	                                    	  if (doc.isCheckedOut()){
	                                    		  writeToLog("{WorkflowDocRetrievalMethod}{execute}Unable to checkout",debug,null);
	                                    		  throw new Exception ("Document is locked or already checkout!!");

	                                    	  }else{
	                                    		  writeToLog("{WorkflowDocRetrievalMethod}{execute}Check out called",debug,null);
	                                    		  doc.checkout();  
	                                    	  }                                  
	                                          if(actName.equalsIgnoreCase("processed")){
		                                    	  doc.setBoolean("fxa_doc_retr_flag",true);
	                                        	  strAuditStatus = "process";
		                                      }
		                                      if(actName.equalsIgnoreCase("rejected")){
		                                      ...
		                                      if(actName.equalsIgnoreCase("NotifyIncomingPool")){
		                                    	//just complete to go to end 
		                                    	 ...
		                                    	 // writeToLog("{WorkflowDocRetrievalMethod}{execute}workflow aborted",debug,null);
		                                      }
		                                                                     
		                                      //doc.save();
		                                      if (doc.isCheckedOut()){
		                                    	  writeToLog("{WorkflowDocRetrievalMethod}{execute}Check in called",debug,null);
		                                    	  doc.checkin(false,"CURRENT");
		                                    	  writeToLog("{WorkflowDocRetrievalMethod}{execute}New ID" +
		                                    			  doc.getObjectId().toString(),debug,null);
		                                      }else{
		                                    	  writeToLog("{WorkflowDocRetrievalMethod}{execute}Unable to checkin",debug,null);
		                                    	  throw new Exception ("Document should be checked in checked out in same session !!");
		                                      }
		                                      writeToLog("{WorkflowDocRetrievalMethod}{execute}Update attribute",debug,null);
		                                      // Update audit table
		                                      writeToLog("{WorkflowDocRetrievalMethod}{execute}Updating Audit Table",debug,null);
		                                	...
		                                	post_value string(50)*/
		                                      ...
		                                      writeToLog("{WorkflowDocRetrievalMethod}{execute}Audit Table updated",debug,null);
	        ...
	        } finally
	        {
	            if ( session != null ){
	                ...
	                writeToLog("{WorkflowDocRetrievalMethod}{execute}Ending ....",debug,null);
	            }
