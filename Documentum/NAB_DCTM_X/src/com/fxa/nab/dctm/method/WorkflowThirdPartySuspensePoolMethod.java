...
public class WorkflowThirdPartySuspensePoolMethod implements IDfMethod
{
	...
	public int execute(Map params, PrintWriter pstream) throws Exception {
    	...
    	writeToLog("{WorkflowThirdPartySuspensePoolMethod}{execute}Starting ....",debug,null);
    	m_writer = pstream;
        ...
        try {
            ...
            writeToLog("{WorkflowThirdPartySuspensePoolMethod}{execute}Getting voucher(s)",debug,null);
                                 ...
                                 if (docIdObj!=null) 
                                    {
                                      ...
                                      String strAuditStatus = "error";
                                      
                                      if(actName.equalsIgnoreCase("End")){
                                    	//just complete to go to end 
                                    	  //doc.setString("fxa_release_flag","COMPLETE");
                                    	  writeToLog("{WorkflowThirdPartySuspensePoolMethod}{execute}workflow complete",debug,null);
                                      }else{
                                    	  if (doc.isCheckedOut()){
                                    		  writeToLog("{WorkflowThirdPartySuspensePoolMethod}{execute}Unable to checkout",debug,null);
                                    		  throw new Exception ("Document is locked or already checkout!!");

                                    	  }else{
                                    		  writeToLog("{WorkflowThirdPartySuspensePoolMethod}{execute}Check out called",debug,null);
                                    		  doc.checkout();  
                                    	  }                                  
                                          if(actName.equalsIgnoreCase("release")){
	                                    	  doc.setString("fxa_release_flag","release");
	                                    	  strAuditStatus = "release";
	                                      }
	                                      if(actName.equalsIgnoreCase("reject")){
	                                    	  doc.setString("fxa_release_flag","reject");
	                                    	  strAuditStatus = "reject";
	                                      }                                      
	                                      ...
	                                      if(actName.equalsIgnoreCase("NotifyIncomingPool")){
	                                    	//just complete to go to end 
	                                    	  ...
	                                    	  strAuditStatus = "reject";
	                                    	  writeToLog("{WorkflowThirdPartySuspensePoolMethod}{execute}workflow aborted",debug,null);
	                                      }
	                                      doc.setString("fxa_tpc_suspense_pool_flag","false");                                      
	                                      //doc.save();
	                                      if (doc.isCheckedOut()){
	                                    	  writeToLog("{WorkflowThirdPartySuspensePoolMethod}{execute}Check in called",debug,null);
	                                    	  doc.checkin(false,"CURRENT");
	                                    	  writeToLog("{WorkflowThirdPartySuspensePoolMethod}{execute}New ID" +
	                                    			  doc.getObjectId().toString(),debug,null);
	                                      }else{
	                                    	  writeToLog("{WorkflowThirdPartySuspensePoolMethod}{execute}Unable to checkin",debug,null);
	                                    	  throw new Exception ("Document should be checked in checked out in same session !!");
	                                      }

	                                      writeToLog("{WorkflowThirdPartySuspensePoolMethod}{execute}Update attribute",debug,null);
	                                      // Update audit table
	                                      writeToLog("{WorkflowSuspensePoolMethod}{execute}Updating Audit Table",debug,null);
	                                      ...
	                                      qryBuff.append("operator_name,");
	                                      qryBuff.append("modified_date");
	                                      qryBuff.append(")");
	                                      ...
	                                      qryBuff.append(",");
	                                      qryBuff.append("'");
	                                      qryBuff.append(m_userName);
	                                      qryBuff.append("'");
	                                      qryBuff.append(",");
	                                      qryBuff.append("DATE(NOW)");	                                      
	                                      qryBuff.append(")");                               
	                                	...
	                                	post_value string(50)*/
	                                      ...
	                                      writeToLog("{WorkflowSuspensePoolMethod}{execute}Audit Table updated",debug,null);
        ...
        } finally
        {
            if ( session != null ){
                ...
                writeToLog("{WorkflowThirdPartySuspensePoolMethod}{execute}Ending ....",debug,null);
            }
    ...
    protected void initWorkflowParams(Map params)
    {
	    ...
	    */
	    protected String getChronicalId(IDfSession session,String objId) throws DfException{
	    	...
	    	writeToLog("{WorkflowDocRetrievalMethod}{getChronicalId}Query {" + qry + "}",debug,null);		
    ...
    protected void updateAuditTable(IDfSession session,String qry) throws DfException{
    	...
    	writeToLog("{WorkflowDocRetrievalMethod}{updateAuditTable}Query {" + qry + "}",debug,null);		
    	IDfQuery query = new DfQuery();
    			query.setDQL(qry);
    			IDfCollection coll = query.execute(session, IDfQuery.DF_QUERY);
    	try{		
