...
public class StoreBatchVoucherTransformTest extends AbstractVoucherProcessingTest {
    ...
    public void shouldStoreBatch_whenTransactionCorrectedValuesForAllVouchers() throws ParseException {
        ...
        for (int i = 0; i < VOUCHER_INPUT_COUNT; i++) {
            ...
            assertThat(voucherProcess.getAlternateAccountNumber(), is(CORRECTED_TRANSACTION_ALTERNATE_ACCOUNT_NUMBER));
            assertThat(voucherProcess.getAlternateAuxDom(), is(CORRECTED_TRANSACTION_ALTERNATE_AUX_DOM));
            assertThat(voucherProcess.getAlternateBsbNumber(), is(CORRECTED_TRANSACTION_ALTERNATE_BSB_NUMBER));
            assertThat(voucherProcess.getAlternateExAuxDom(), is(CORRECTED_TRANSACTION_ALTERNATE_EX_AUX_DOM));
            assertThat(voucherProcess.getAlternateTransactionCode(), is(CORRECTED_TRANSACTION_ALTERNATE_TRANSACTION_CODE));
    ...
    public void shouldStoreBatch_whenWorkTypeIsAustraliaPost() throws ParseException {
        ...
        job.setJobIdentifier(JOB_IDENTIFIER);
        job.getActivities().add(craftReceivedFileActivity());
        job.getActivities().add(craftUnpackageBatchVoucherActivity(WorkTypeEnum.NABCHQ_APOST, null));
        job.getActivities().add(craftCorrectCodelineActivity(1));
        ...
        assertHeader(request, WorkTypeEnum.NABCHQ_APOST);
        ...
        for (int i = 0; i < VOUCHER_INPUT_COUNT; i++) {
            ...
            StoreVoucherRequest storeVoucherRequest = request.getVouchers().get(i);
            VoucherBatch voucherBatch = request.getVoucherBatch();
            assertVoucherTransfer(storeVoucherRequest, voucherBatch);
    ...
    @Test
    public void shouldStoreBatch_whenThirdPartyCheckFailed() throws ParseException {
        StoreBatchVoucherTransform target = new StoreBatchVoucherTransform();
        ...
        job.getActivities().add(craftValidateBatchTransactionActivity());
        job.getActivities().add(craftCorrectBatchTransactionActivity());
        job.getActivities().add(craftCheckThirdPartyActivityWithTPCFailed());
    ...
    private void assertHeader(StoreBatchVoucherRequest request, WorkTypeEnum workType) {
        assertThat(request.getJobIdentifier(), is(JOB_IDENTIFIER));
        assertThat(request.getOrigin(), is(DocumentExchangeEnum.VOUCHER_INBOUND));
        ReceivedFile receipt = request.getReceipt();
        assertThat(receipt, is(notNullValue()));
        ...
        assertThat(voucherBatch.getCollectingBank(), is(COLLECTING_BANK));
        assertThat(voucherBatch.getCaptureBsb(), is(CAPTURE_BSB));
        assertThat(voucherBatch.getWorkType(), is(workType));

    }

    private void assertVoucherTransfer(StoreVoucherRequest request, VoucherBatch voucherBatch) {
        int endPointCount = request.getTransferEndpoints().size();

        for (int i = 0; i < endPointCount; i++) {
            TransferEndpoint endpoint = (TransferEndpoint) request.getTransferEndpoints().get(i);
            String status = endpoint.getVoucherStatus().value();
            String targetEndpoint = endpoint.getEndpoint();
            String documentExchange = endpoint.getDocumentExchange().value();
            String workType = voucherBatch.getWorkType().value();
            String batchType = voucherBatch.getBatchType();
            if (workType.equals(WorkTypeEnum.NABCHQ_APOST) ||
                    request.getVoucherProcess().isThirdPartyCheckFailed()){
                assertThat(status, is(VoucherStatus.ON_HOLD.value()));
            }

            if ((documentExchange.equals(DocumentExchangeEnum.VIF_OUTBOUND.value())) || (documentExchange.equals(DocumentExchangeEnum.VIF_ACK_OUTBOUND.value()))) {
                if (workType.equals(WorkTypeEnum.NABCHQ_APOST.value())) {
                    assertThat(targetEndpoint, is(voucherBatch.getWorkType() + "::" + voucherBatch.getCaptureBsb()));
                } else if (workType.equals((WorkTypeEnum.NABCHQ_LBOX.value()))) {
                    String test = voucherBatch.getWorkType().value() + ":" + batchType + ":" + voucherBatch.getCaptureBsb();
                    assertThat(targetEndpoint, is(voucherBatch.getWorkType().value() + ":" + batchType + ":" + voucherBatch.getCaptureBsb()));
                }
            }
        }
    }
    ...
    @Test
    public void shouldStoreBatch_whenWorkTypeIsLockedBox() throws ParseException {
        StoreBatchVoucherTransform target = new StoreBatchVoucherTransform();

        Job job = new Job();
        job.setJobIdentifier(JOB_IDENTIFIER);
        job.getActivities().add(craftReceivedFileActivity());
        job.getActivities().add(craftUnpackageBatchVoucherActivity(WorkTypeEnum.NABCHQ_LBOX, "LCS1"));
        job.getActivities().add(craftCorrectCodelineActivity(1));
        ...
        assertHeader(request, WorkTypeEnum.NABCHQ_LBOX);
