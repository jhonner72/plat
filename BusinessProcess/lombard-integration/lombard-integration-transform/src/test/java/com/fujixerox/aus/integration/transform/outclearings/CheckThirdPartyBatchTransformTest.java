...
public class CheckThirdPartyBatchTransformTest extends AbstractVoucherProcessingTest {
	...
	public void shouldTransform_whenExpertBalancingHappen() throws ParseException {
		...
		for (int i = 0; i < VOUCHER_INPUT_COUNT; i++) {
			...
			assertThat(checkThirdPartyRequest.getVoucherProcess().getAlternateAccountNumber(), is(CORRECTED_TRANSACTION_ALTERNATE_ACCOUNT_NUMBER));
			assertThat(checkThirdPartyRequest.getVoucherProcess().getAlternateAuxDom(), is(CORRECTED_TRANSACTION_ALTERNATE_AUX_DOM));
			assertThat(checkThirdPartyRequest.getVoucherProcess().getAlternateBsbNumber(), is(CORRECTED_TRANSACTION_ALTERNATE_BSB_NUMBER));
			assertThat(checkThirdPartyRequest.getVoucherProcess().getAlternateExAuxDom(), is(CORRECTED_TRANSACTION_ALTERNATE_EX_AUX_DOM));
			assertThat(checkThirdPartyRequest.getVoucherProcess().getAlternateTransactionCode(), is(CORRECTED_TRANSACTION_ALTERNATE_TRANSACTION_CODE));
		}
	}
