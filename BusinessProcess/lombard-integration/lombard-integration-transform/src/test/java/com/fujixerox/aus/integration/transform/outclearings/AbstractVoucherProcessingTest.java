...
public class AbstractVoucherProcessingTest {
    ...
    //Lockecd Box
    protected static final String BATCH_TYPE = "LCG1";
    protected static final String SUB_BATCH_TYPE = "LPS";
    protected static final boolean CREDIT_NOTE_FLAG_FROM_UNPACK = false;
    protected static final boolean CREDIT_NOTE_FLAG_FROM_CORRECT_CODELINE = true;
    private static final boolean CREDIT_NOTE_FLAG_WITH_CHANGE = false;
    ...
    protected static final String SCANNED_BATCH_AUX_DOM = "33";
    ...
    protected static final boolean CORRECTED_UNENCODED_ECD = false;
    ...
    protected static final String CORRECTED_TRANSACTION_ALTERNATE_ACCOUNT_NUMBER = "11111111";
    protected static final String CORRECTED_TRANSACTION_ALTERNATE_AUX_DOM = "11";
    protected static final String CORRECTED_TRANSACTION_ALTERNATE_BSB_NUMBER = "222222";
    protected static final String CORRECTED_TRANSACTION_ALTERNATE_EX_AUX_DOM = "99";
    protected static final String CORRECTED_TRANSACTION_ALTERNATE_TRANSACTION_CODE = "777";
    ...
    protected Activity craftUnpackageBatchVoucherActivity() throws ParseException {
        ...
        for (int i = 0; i < VOUCHER_INPUT_COUNT; i++) {
            ...
            voucher.setCreditNoteFlag(CREDIT_NOTE_FLAG_FROM_UNPACK);
    ...
    protected Activity craftUnpackageBatchVoucherActivityForLockedBoxLPS() throws ParseException {
        Activity unpackageActivity = new Activity();
        unpackageActivity.setPredicate("unpackage");
        unpackageActivity.setSubject("voucher");
        UnpackageBatchVoucherResponse response = new UnpackageBatchVoucherResponse();
        ...
        //Locked Box
        ScannedBatch scannedBatch = createScannedBatchForLockedBox();
        ...
        for (int i = 0; i < VOUCHER_INPUT_COUNT; i++) {
            ...
            voucher.setExtraAuxDom(SCANNED_EXTRA_AUX_DOM);
            voucher.setAccountNumber(SCANNED_ACCOUNT_NUMBER);
            voucher.setAmount(CAPTURED_AMOUNT);
            voucher.setProcessingDate(PROCESSING_DATE);
            voucher.setInactiveFlag(INACTIVE_FLAG_FALSE);
            voucher.setMicrFlag(MICR_FLAG_TRUE);
            voucher.setCreditNoteFlag(CREDIT_NOTE_FLAG_FROM_UNPACK);
    ...
    protected Activity craftRecogniseBatchCourtesyAmountActivityOnlyForActiveVouchers(List<String> activeDocumentReferenceNumbers) throws ParseException {
        ...
        unpackageActivity.setPredicate("recognise");
        unpackageActivity.setSubject("courtesyamount");
        unpackageActivity.setRequestDateTime(new SimpleDateFormat(ISO_DATE).parse(EXPECTED_DATE));
        unpackageActivity.setResponseDateTime(new SimpleDateFormat(ISO_DATE).parse(EXPECTED_DATE));
        RecogniseBatchCourtesyAmountResponse response = new RecogniseBatchCourtesyAmountResponse();
    ...
    protected Activity craftCorrectCodelineActivity(int voucherExclude) throws ParseException {
        ...
        for (int i = 0; i < VOUCHER_INPUT_COUNT; i++) {
            ...
            correctCodelineResponse.setCreditNoteFlag(CREDIT_NOTE_FLAG_FROM_CORRECT_CODELINE);
    ...
    protected Activity craftCorrectCodelineActivityWithInactiveAndActiveVouchers(List<String> activeDrns, String exclude) throws ParseException {
        ...
        for (String drn : activeDrns) {
            ...
            correctCodelineResponse.setPostTransmissionQaCodelineFlag(POST_TRANSMISSION_QA_CODELINE_FLAG);
            correctCodelineResponse.setPostTransmissionQaAmountFlag(POST_TRANSMISSION_QA_AMOUNT_FLAG);
            correctCodelineResponse.setCreditNoteFlag(CREDIT_NOTE_FLAG_WITH_CHANGE);
            vouchers.add(correctCodelineResponse);
    ...
    protected Activity craftCorrectBatchTransactionActivity() {
        ...
        for (int i = 0; i < VOUCHER_INPUT_COUNT; i++)
        {
            ...
            correctTransactionResponse.setAlternateAccountNumber(CORRECTED_TRANSACTION_ALTERNATE_ACCOUNT_NUMBER);
            correctTransactionResponse.setAlternateAuxDom(CORRECTED_TRANSACTION_ALTERNATE_AUX_DOM);
            correctTransactionResponse.setAlternateBsbNumber(CORRECTED_TRANSACTION_ALTERNATE_BSB_NUMBER);
            correctTransactionResponse.setAlternateExAuxDom(CORRECTED_TRANSACTION_ALTERNATE_EX_AUX_DOM);
            correctTransactionResponse.setAlternateTransactionCode(CORRECTED_TRANSACTION_ALTERNATE_TRANSACTION_CODE);
            correctTransactionResponse.setCreditNoteFlag(CREDIT_NOTE_FLAG_FROM_CORRECT_CODELINE);
    ...
    protected Activity craftCheckThirdPartyActivityWithTPCFailed() {
        Activity checkThirdPartyActivity = new Activity();
        checkThirdPartyActivity.setPredicate("check");
        checkThirdPartyActivity.setSubject("thirdparty");
        checkThirdPartyActivity.setRequestDateTime(new Date());
        checkThirdPartyActivity.setResponseDateTime(new Date());

        CheckThirdPartyBatchResponse response = new CheckThirdPartyBatchResponse();
        VoucherBatch voucherBatch =new VoucherBatch();
        voucherBatch.setScannedBatchNumber(BATCH_NUMBER);
        voucherBatch.setBatchAccountNumber(BATCH_ACCOUNT_NUMBER);
        voucherBatch.setProcessingState(PROCESSING_STATE);
        ...
        CheckThirdPartyBatchRequest request = new CheckThirdPartyBatchRequest();
        request.setVoucherBatch(voucherBatch);


        List<CheckThirdPartyResponse> vouchers = response.getVouchers();

        for (int i = 0; i < VOUCHER_INPUT_COUNT; i++)
        {
            CheckThirdPartyResponse checkThirdPartyResponse = new CheckThirdPartyResponse();
            ...
            voucher.setExtraAuxDom(i != 1 ? CORRECTED_TRANSACTION_EXTRA_AUX_DOM : CORRECTED_EXTRA_AUX_DOM);
            voucher.setAmount(i != 1 ? CORRECTED_TRANSACTION_AMOUNT : CORRECTED_AMOUNT);
            checkThirdPartyResponse.setVoucher(voucher);
            vouchers.add(checkThirdPartyResponse);

            VoucherProcess voucherProcess = new VoucherProcess();
            voucherProcess.setThirdPartyCheckFailed(false);
            voucherProcess.setTransactionLinkNumber(CORRECTED_TRANSACTION_TRANSACTION_LINK_NUMBER);
            voucherProcess.setAdjustedBy(ADJUSTED_BY);
            voucherProcess.setAdjustedFlag(ADJUSTED_FLAG);
            voucherProcess.setAdjustmentDescription(ADJUSTMENT_DESCRIPTION);
            voucherProcess.setAdjustmentLetterRequired(ADJUSTMENT_LETTER_REQUIRED);
            voucherProcess.setAdjustmentReasonCode(ADJUSTED_REASON_CODE);
            voucherProcess.setVoucherDelayedIndicator(VOUCHER_DELAYED_INDICATOR);
            ...
            checkThirdPartyResponse.setVoucherProcess(voucherProcess);
        }


        List<CheckThirdPartyRequest> vouchersReq = request.getVouchers();
        for (int i = 0; i < VOUCHER_INPUT_COUNT; i++)
        {
            CheckThirdPartyRequest checkThirdPartyRequest = createCheckThirdPartyRequest();
            Voucher voucher = new Voucher();
            voucher.setDocumentReferenceNumber(DOCUMENT_REFERENCE_NUMBER + i);
            voucher.setDocumentType(DocumentTypeEnum.CRT);
            voucher.setProcessingDate(PROCESSING_DATE);
            ...
            checkThirdPartyRequest.setThirdPartyCheckRequired(true);
            vouchersReq.add(checkThirdPartyRequest);
        }
        checkThirdPartyActivity.setRequest(request);
        checkThirdPartyActivity.setResponse(response);
        return checkThirdPartyActivity;
    ...
    protected CorrectTransactionRequest createCorrectTransactionRequest(
        ...
        correctTransactionRequest.setPostTransmissionQaCodelineFlag(POST_TRANSMISSION_QA_CODELINE_FLAG);
        correctTransactionRequest.setPostTransmissionQaAmountFlag(POST_TRANSMISSION_QA_AMOUNT_FLAG);
        correctTransactionRequest.setCreditNoteFlag(CREDIT_NOTE_FLAG_FROM_CORRECT_CODELINE);

        correctTransactionRequest.setIsGeneratedVoucher(false);
        return correctTransactionRequest;
    }

    protected CheckThirdPartyRequest createCheckThirdPartyRequest() {
        CheckThirdPartyRequest checkThirdPartyRequest = new CheckThirdPartyRequest();
        checkThirdPartyRequest.setThirdPartyCheckRequired(true);
        return checkThirdPartyRequest;
    }
    ...
    private ScannedBatch createScannedBatch() throws ParseException {
        ...
        scannedBatch.setBatchType("");
        scannedBatch.setSubBatchType("");
        scannedBatch.setBatchAuxDom(SCANNED_BATCH_AUX_DOM);
		return scannedBatch;
	}
    
    private ScannedBatch createScannedBatchForLockedBox() throws ParseException {
		ScannedBatch scannedBatch = new ScannedBatch();
        scannedBatch.setProcessingDate(new SimpleDateFormat(ISO_DATE).parse(EXPECTED_DATE));
        scannedBatch.setBatchNumber(BATCH_NUMBER);
        scannedBatch.setUnitID(UNIT_ID);
        ...
        scannedBatch.setBatchAccountNumber(BATCH_ACCOUNT_NUMBER);
        //LockedBox ListPays
        scannedBatch.setBatchType(BATCH_TYPE);
        scannedBatch.setSubBatchType(SUB_BATCH_TYPE);
        scannedBatch.setBatchAuxDom(SCANNED_BATCH_AUX_DOM);
		return scannedBatch;
	}
    ...
    protected CorrectCodelineRequest createCorrectCodeLineRequest(String drn) {
        ...
        correctCodelineRequest.setCreditNoteFlag(CREDIT_NOTE_FLAG_FROM_CORRECT_CODELINE);

        return correctCodelineRequest;
    }

    protected Activity craftUnpackageBatchVoucherActivity(WorkTypeEnum workType, String batchType) throws ParseException {
        Activity unpackageActivity = new Activity();
        unpackageActivity.setPredicate("unpackage");
        unpackageActivity.setSubject("voucher");
        UnpackageBatchVoucherResponse response = new UnpackageBatchVoucherResponse();

        ScannedBatch scannedBatch = createScannedBatch(workType, batchType);
    ...
    private ScannedBatch createScannedBatch(WorkTypeEnum workType, String batchType) throws ParseException {
        ScannedBatch scannedBatch = new ScannedBatch();
        scannedBatch.setProcessingDate(new SimpleDateFormat(ISO_DATE).parse(EXPECTED_DATE));
        scannedBatch.setBatchNumber(BATCH_NUMBER);
        scannedBatch.setUnitID(UNIT_ID);
        ...
        scannedBatch.setCollectingBank(COLLECTING_BANK);
        scannedBatch.setCaptureBsb(CAPTURE_BSB);
        scannedBatch.setWorkType(workType);
        scannedBatch.setBatchType(batchType);
        scannedBatch.setBatchAccountNumber(BATCH_ACCOUNT_NUMBER);
        return scannedBatch;
    }
