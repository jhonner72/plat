...
public class CreateBatchValueInstructionFileTransform extends AbstractOutclearingsTransform implements Transformer <CreateValueInstructionFileRequest> {
    ...
    public CreateValueInstructionFileRequest transform(Job job) {
        ...
        if (jobIdentifier.startsWith("NECL") || jobIdentifier.startsWith("NLBC")) {
            request.setQueryLinkType(com.fujixerox.aus.lombard.common.voucher.QueryLinkTypeEnum.CUSTOMER_LINK_NUMBER);
        } else {
            request.setQueryLinkType(com.fujixerox.aus.lombard.common.voucher.QueryLinkTypeEnum.TRANSACTION_LINK_NUMBER);
        }
