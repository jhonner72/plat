...
public class ForValueCorrectBatchCodelineTransform extends AbstractTransform implements Transformer <CorrectBatchCodelineRequest>{
    ...
    private CorrectCodelineRequest mapVoucher(VoucherInformation voucherInformation) {
        ...
        request.setDocumentReferenceNumber(voucher.getDocumentReferenceNumber()); // Note: Long DRN's will get regenerated internally going into DIPS
        ...
        /*
        Fix 21226 - as DIPS only supports 9-digit DRN's (database/system limit), then a 12-digit
        field was added that can hold this long DRN's (i.e. transactionId's from foreign banks
        used during inwards for value processing).
         */
        ...
        // As no Codeline Validation occurs during inclearings, all status flags default to false
    ...
    private String setDipsDocType(String docType) {
        ...
        else if (docType.equalsIgnoreCase("Sp")) documentType = "SP"; // Not relevant in this context anyway
