...
public class UpdateVouchersInformationRequestTransform {  
	...
	public void transform(VoucherInformation voucherInfo, IDfSysObject fxaVoucher, String mappingHandlerType) throws DfException  {
		
		Voucher voucher = voucherInfo.getVoucher();
	    LogUtil.log("UpdateVouchersInformationRequestTransform for VoucherInformation: " + voucher.getDocumentReferenceNumber(), LogUtil.INFO, null);
		...
		WorkTypeEnum workType = null;
		if (voucherBatch != null) {
			workType = voucherBatch.getWorkType();
		}
		
	    try {
	    	MappingEntryFxaProcessor mappingEntryFxaProcessor = new MappingEntryFxaProcessor();
	    	MappingSAXHandler handler = MappingSAXHandlerFactory.getHandler(mappingHandlerType, workType);
	...
	public void transform(List<Criteria> criterias, IDfSysObject fxaVoucher) throws DfException  {
		
	    LogUtil.log("UpdateVouchersInformationRequestTransform for Criteria", LogUtil.INFO, null);
	    MappingEntry lastEntry = null;
	    
	    try {
	    	MappingEntryFxaProcessor mappingEntryFxaProcessor = new MappingEntryFxaProcessor();
	    	MappingSAXHandler handler = MappingSAXHandlerFactory.getHandler();	    	
		    for ( Criteria criteria : criterias) {
		    	lastEntry = handler.getMappingEntryByValue(criteria.getName());		    	
		    	mappingEntryFxaProcessor.process(criteria, fxaVoucher, lastEntry);
		    }
		} catch (Exception e) {
			LogUtil.log("UpdateVouchersInformationRequestTransform -- error in transform " + lastEntry.getKey(), LogUtil.ERROR, null);
			throw new DfException(e);
		}
	}
	
/*	public void transform_old(VoucherInformation voucherInfo, IDfSysObject fxaVoucher) throws DfException  {
