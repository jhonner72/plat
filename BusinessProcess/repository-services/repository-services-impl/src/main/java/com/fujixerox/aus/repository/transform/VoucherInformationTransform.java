...
public class VoucherInformationTransform {
	...
	public VoucherInformation transform(IDfSysObject fxaVoucher) throws DfException   {
		...
		try {
			MappingEntryObjectProcessor mappingEntryObjectProcessor = new MappingEntryObjectProcessor();
			MappingSAXHandler handler = MappingSAXHandlerFactory.getHandler();
		    for ( MappingEntry entry : handler.getEntryList()) {
		    	mappingEntryObjectProcessor.process(fxaVoucher, voucherInfo, entry);
		    }
		} catch (Exception e) {
