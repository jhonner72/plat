...
public class DocumentumACL {	
    ...
    private static Map<String, IDfFolder> dayFolderPathMap;
    ...
    private static void init() {
    	...
    	dayFolderPathMap = new HashMap<String, IDfFolder>();
    	
    	LogUtil.log("DocumentumACL Instantiated !! ", LogUtil.INFO, null);
    }
	
	public synchronized IDfACL getACL(IDfSession session, String aclName) throws DfException, ACLException {	
		...
		if (thisACL == null) {
			...
			LogUtil.log("DocumentumACL new aclName:"+aclName+" cached..", LogUtil.DEBUG, null);
		} 
		
		return thisACL;
	}

	public synchronized String getIdForDateFolder(IDfSession session, String folderAclName, String parentFolderPath, Date processingDate)
        ...
        String dayFolderPath = getDayFolderPath(parentFolderPath, processingDate);
        IDfFolder dayFolderObj = dayFolderPathMap.get(dayFolderPath);
		if (dayFolderObj == null) {
			dayFolderObj = (IDfFolder)session.getFolderByPath(dayFolderPath);
	        // If folder path does not exist then create
			if (dayFolderObj == null) {
				//LogUtil.log(("No folder exist with the given path '" + dayFolderPath + "', so creating", LogUtil.INFO, null);
				String folderPath = checkFolderExist(session, folderAclName, parentFolderPath, processingDate);
				dayFolderObj = (IDfFolder)session.getFolderByPath(folderPath);
			}
			
			dayFolderPathMap.put(dayFolderPath, dayFolderObj);
			
			LogUtil.log("DocumentumACL new dayFolderPath:"+dayFolderPath+" : "+dayFolderObj.getObjectId().toString()+" cached..", LogUtil.DEBUG, null);
		}
		String dayFolderObjId = dayFolderObj.getObjectId().toString();

		return dayFolderObjId;

	}

	private String getDayFolderPath(String parentFolderPath, Date processingDate) {
		// Determine the path base on the processing date
