...
public class CompatRoute extends RouteBuilder {
    ...
    public CompatRoute(String contextPath, IRequestProcessor loginProcessor,
            IRequestProcessor logoutProcessor,
            IRequestProcessor indexQueryProcessor,
            IRequestProcessor fallBackProcessor) {
    ...
    public CompatRoute(String contextPath, IRequestProcessor loginProcessor,
            IRequestProcessor logoutProcessor,
            IRequestProcessor indexQueryProcessor,
            IRequestProcessor fallBackProcessor, String logLevel,
    ...
    public void configure() throws Exception {
        ...
        onException(Throwable.class).handled(true).to(getLog(true))
                .removeHeaders("*").to(EndPoints.FALLBACK.getName())
                .id(makeId(EndPoints.FALLBACK));

        from(getStart()).to(getLog()).removeHeaders("*").choice().when()
                .xpath(EndPoints.LOGIN.getXPath())
                .to(EndPoints.LOGIN.getName()).id(makeId(EndPoints.LOGIN))
                .when().xpath(EndPoints.LOGOUT.getXPath())
                .to(EndPoints.LOGOUT.getName()).id(makeId(EndPoints.LOGOUT))
                .when().xpath(EndPoints.INQUIRY.getXPath())
                .to(EndPoints.INQUIRY.getName()).id(makeId(EndPoints.INQUIRY))
                .otherwise().to(EndPoints.FALLBACK.getName())
                .id(makeId(EndPoints.FALLBACK)).endChoice().end();
        ...
        from(EndPoints.INQUIRY.getName()).choice().when()
                .xpath(EndPoints.INDEXQUERY.getXPath())
                .to(EndPoints.INDEXQUERY.getName())
                .id(makeId(EndPoints.INDEXQUERY)).otherwise()
                .to(EndPoints.FALLBACK.getName())
                .id(makeId(EndPoints.FALLBACK)).end();
        ...
        from(EndPoints.INDEXQUERY.getName())
                ...
                .id(makeId(EndPoints.INDEXQUERYRESULT_MARSHALL)).end();
        ...
        from(EndPoints.FALLBACK.getName()).bean(_fallBackProcessor, METHOD)
                .to(EndPoints.RESPONSE.getName())
                .id(makeId(EndPoints.RESPONSE)).end();

        from(EndPoints.RESPONSE.getName())
                .setHeader("Content-type", simple("test/xml")).to(getLog())
                .to("mock:result?retainFirst=0&retainLast=0");
    ...
    private String getLog(boolean isError) {
        ...
        if (isError) {
            log += String
                    .format("&level=%s&showException=%s&showCaughtException=%s&showStackTrace=%s",
    ...
    public DataFormat getInputFormat(IRequestProcessor processor) {
        JaxbDataFormat format = new JaxbDataFormat(processor.getRequestClass()
                .getPackage().getName());
    ...
    public DataFormat getOutputFormat(IRequestProcessor processor) {
        JaxbDataFormat format = new JaxbDataFormat(processor.getResponseClass()
                .getPackage().getName());
        format.setMustBeJAXBElement(false);
        return format;
    }
