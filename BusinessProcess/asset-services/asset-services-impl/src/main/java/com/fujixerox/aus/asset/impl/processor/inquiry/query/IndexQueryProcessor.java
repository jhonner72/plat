...
public class IndexQueryProcessor extends AbstractInquiryProcessor {
    ...
    protected Inquiryresult doProcess(Inquiry request, String userName) {
        ...
        for (IndexQuery query : request.getIndexqueries()) {
            result.getIndexqueryresults().add(
                    processSearchRequest(request, query));
        }
    ...
    private IndexQueryResult processSearchRequest(Inquiry request,
            IndexQuery query) {
        IndexQueryResult result = new IndexQueryResult().withQueryname(query
                .getQueryname());
        ...
        try {
            ...
            boolean hasMore = invokeInSession(request.getSecurityToken(),
                    new IndexQueryInvoker(request, query, result,
                            getAttributeMapping(), getFallbackDataProvider(),
                            maxRows));
