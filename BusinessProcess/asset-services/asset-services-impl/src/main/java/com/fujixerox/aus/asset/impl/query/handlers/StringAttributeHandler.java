    ...
    protected String doGetRestriction(IDfSession session, String qualification,
        ...
        if (containsWildcards) {
            if (isWildCard()) {
                ...
                Logger
                        .debug("Request contains wildcards but wildcards are not supported");
        ...
        if (containsWildcards) {
            return like(qualification,
                    toQuotedString(replaceWildcards(lowValue)), matchCase);
        }
        if (!isRange()) {
            Logger.debug("Range is not supported");
            throw new IllegalArgumentException(
                    "Unable to build restriction for [" + lowValue + ";"
                            + highValue + "]");
        }
        if (StringUtils.isEmpty(lowValue)) {
            Logger.debug("Low ''{0}'' is blank", lowValue);
            return lessThanOrEqual(qualification, toQuotedString(highValue),
                    matchCase);
        }
        if (StringUtils.isEmpty(highValue)) {
            Logger.debug("High ''{0}'' is blank", highValue);
            return greaterThanOrEqual(qualification, toQuotedString(lowValue),
                    matchCase);
        }
        return betweenOrEqual(qualification, toQuotedString(lowValue),
                toQuotedString(highValue), matchCase);
    }
