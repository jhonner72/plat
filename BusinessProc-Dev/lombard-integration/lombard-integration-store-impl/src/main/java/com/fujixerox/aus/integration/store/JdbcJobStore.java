...
//    }
    ...
    {
        List<String> jobIdList = template.queryForList("select JOB_ID from JOB where DATEDIFF(day, MODIFIED_DATE, GETDATE()) > ?", new Object[]{assetRetention.getRetentionDays()}, String.class);
        return jobIdList;
    }
    ...
    public int deleteJob(AssetRetention assetRetention) {
        int recordCount;
        recordCount = template.update("delete from JOB where DATEDIFF(day, MODIFIED_DATE, GETDATE()) > ?", new Object[]{assetRetention.getRetentionDays()});
        return recordCount;
    }
