...
public aspect ComponentTestAspect {
    ...
    public final void setupJNDIContext(IComponentTest target) {
        ClassPathXmlApplicationContext ctx = new ClassPathXmlApplicationContext("classpath:/spring/test-jndi.xml");
        ContextBuilder jndiLoader = (ContextBuilder) ctx.getBean("jndiLoader");
    }
    ...
    public final String IComponentTest.getUserName() {
        return _internalCredentialManager.getCredentials(null, null).getUserName();
    }

    public final String IComponentTest.getPassword() {
        return _internalCredentialManager.getCredentials(null, null).getPassword();
    }
