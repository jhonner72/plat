...
public abstract class AbstractCompatRouteTest extends AbstractCamelTest {
    ...
    protected final void doPreSetup() throws Exception {
        ...
        _loginProcessor = new LoginProcessor(credentialManager);
        _logoutProcessor = new LogoutProcessor(credentialManager, false);
        _indexQueryProcessor = new IndexQueryProcessor(credentialManager, false, getMapping(), 
        		new BeanFallbackDataProvider("images/blank.png", "images/nodata.png", "images/nodata.png", null));
    ...
    protected final MockEndpoint replace(final int routerNo, final SimpleEndPoint endPoint) throws Exception {
        return replace(routerNo, new SimpleEndPoint[] {endPoint, }).get(0);
    }

    protected final List<MockEndpoint> replace(final int routerNo, final SimpleEndPoint... endPoints) throws Exception {
        List<MockEndpoint> result = new ArrayList<>();
        for (SimpleEndPoint endPoint : endPoints) {
            MockEndpoint mockEndPoint = getMockEndpoint(getMockName(endPoint.getId()), true);
        ...
        AdviceWithRouteBuilder adviceWithRouteBuilder = new AdviceWithRouteBuilder() {
            ...
            public void configure() throws Exception {
                for (SimpleEndPoint endPoint : endPoints) {
                    weaveById(String.format("%s:%d", endPoint.getId(), routerNo)).replace().to(getMockName(endPoint.getId()));
                }
            }
        };

        context.getRouteDefinitions().get(routerNo - 1).adviceWith(context, adviceWithRouteBuilder);
    ...
    protected IAttributeMapping getMapping() {
        ...
        return (IAttributeMapping) ctx.getBean("cvolViproDctmFxaVoucherMapping");
    }

    @Override
    public ICredentialManager getCredentialManager() {
        ClassPathXmlApplicationContext ctx = new ClassPathXmlApplicationContext(
                "classpath:/spring/auth/shiro-preferences-test.xml");
        return (ICredentialManager) ctx.getBean("credentialManager");
    }

    @Override
    protected final RouteBuilder createRouteBuilder() throws Exception {
        return new CompatRoute("xml", _loginProcessor, _logoutProcessor, _indexQueryProcessor, _fallBackProcessor) {
            ...
            public DataFormat getInputFormat(IRequestProcessor processor) {
                JaxbDataFormat format = new JaxbDataFormat(RequestBase.class.getPackage().getName());
            ...
            public DataFormat getOutputFormat(IRequestProcessor processor) {
                JaxbDataFormat format = new JaxbDataFormat(ResponseBase.class.getPackage().getName());
                format.setMustBeJAXBElement(false);
                return format;
            }
