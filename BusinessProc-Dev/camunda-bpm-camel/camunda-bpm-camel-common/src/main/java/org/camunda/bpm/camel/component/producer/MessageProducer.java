...
public class MessageProducer extends CamundaBpmProducer {
  ...
  public void process(Exchange exchange) throws Exception {    
    ...
    String messageName = exchange.getProperty("messageName", String.class);
    if (messageName == null) {
      messageName = this.messageName;
    }

    if (processInstanceId == null && businessKey!=null) {
      processInstanceId = queryProcessInstanceResielent(businessKey, messageName);
    }
  ...
  private String queryProcessInstanceResielent(String businessKey, String messageName)
  {
    for (int i = 0; i < 20; i++) {
      String processInstanceId = queryProcessInstanceId(businessKey);
      if (processInstanceId != null) {
      ...
      {
        try {
          log.warn("Waiting to find process for key=" + businessKey + " message=" + messageName);
          businessKey.wait(500);
        } catch (InterruptedException e) {
          break;
        }
      }
    }
    log.error("Giving up waiting for process for key=" + businessKey + " message=" + messageName);
    throw new RuntimeException("Giving up trying to find process with business key:" + businessKey);
  }

  private String queryProcessInstanceId(String businessKey) {
